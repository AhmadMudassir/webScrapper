...
function convertToCSV(data) {
  ...
  const rows = data.map(obj => {
    // handle commas within Supply field by enclosing it in double quotes
    const supply = obj.Supply ? `"${obj.Supply}"` : "";
    // create rows from object values, except for the Supply field which has already been processed
    const values = Object.values(obj).map((val, index) => index === 5 ? supply : val);
    return values.join(',');
  }).join('\n');
  return header + rows;
}
