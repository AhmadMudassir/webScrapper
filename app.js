...
app.post('/submit', (req, res) => {
    ...
    try {
        (async () => {
            ...
            for (let i = startingRollNo; i <= endingRollNo; i++) {
                ...
                console.log(noSpaces)
                  ...
                  if (noSpaces.includes('(PrvFailin')) {
                    // Without Spaces
                    const cgpaMatch = noSpaces.match(/TOTALCGPA(\d+\.\d+)\s*\((PrvFailin\S*?\)\)).*?Back/i);
            ...
            // }
                // const cpgamatch = noSpaces.match(/CGPA([\d.]+)/i);
                // let cgpa = null;
                // if (cpgamatch !== null) {
                // cgpa = Number(cpgamatch[1]);
                // }

                // // Get Supply
                // const supplymatch = noSpaces.match(/(PrvFail\S*)BACK/);
                // let supply = null;
                // if (supplymatch !== null) {
